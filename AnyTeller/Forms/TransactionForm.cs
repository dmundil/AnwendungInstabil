    ...
    public partial class TransactionForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            btnSubmit = new Button
            {
                ...
                Location = new Point(650, 540), // Moved down below GroupBox (80+450=530)
        ...
        {
            if (txtSearch.Text.Trim().Equals(_currentRecord.Username, StringComparison.OrdinalIgnoreCase))
            {
                // Found
                SetupChaosForm(true);
            }
            ...
            {
                MessageBox.Show("User not found or does not match current queue priority.", "Lookup Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private (Control lbl, Control input, bool isPlaceholder) CreateReasonWidgetControls(Random rnd)
        {
            ...
            if (!usePlaceholder)
            {
                lbl = new Label { Text = synonym, AutoSize = true };
                lbl.TextAlign = ContentAlignment.MiddleLeft;
                lbl.Anchor = AnchorStyles.Left | AnchorStyles.Right;
                lbl.Margin = new Padding(0, 5, 10, 5);
            }
            ...
            {
                var txt = new TextBox { Text = "", Name = "txtReason" };
                if (usePlaceholder)
                {
                    // Basic placeholder simulation using Text (since WinForms TextBox doesn't support native PlaceholderText until .NET Core 3+ / 5+ or via SendMessage)
                    // The factory might have done this? 
                    // Let's assume .NET 8 supports PlaceholderText.
            ...
            {
                var cb = new ComboBox { Name = "cbReason", DropDownStyle = ComboBoxStyle.DropDownList };
                cb.SelectedIndex = -1;
            ...
            {
                var flp = new FlowLayoutPanel { AutoSize = true, FlowDirection = FlowDirection.TopDown, Name = "pnlReason" };
                ...
                {
                    var rb = new RadioButton { Text = opt, AutoSize = true, Checked = false };
                    flp.Controls.Add(rb);
                }
